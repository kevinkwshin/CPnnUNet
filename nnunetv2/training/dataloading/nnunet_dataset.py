...
class nnUNetDatasetNumpy(nnUNetBaseDataset):
    def load_case(self, identifier, target_label_file: str = None):
        ...
        if target_label_file:
            seg_file = join(self.source_folder, target_label_file)
        else:
            seg_file = join(self.source_folder, identifier + '_seg.npy')

        if not isfile(seg_file):
            # fallback to .npz for segmentation
            if target_label_file:
                npz_file = join(self.source_folder, target_label_file.replace('.npy', '.npz'))
            else:
                npz_file = join(self.source_folder, identifier + '.npz')
            seg = np.load(npz_file)['seg']
        else:
            seg = np.load(seg_file, mmap_mode='r')
...
class nnUNetDatasetBlosc2(nnUNetBaseDataset):
    ...
    def load_case(self, identifier, target_label_file: str = None):
        ...
        if target_label_file:
            seg_b2nd_file = join(self.source_folder, target_label_file)
        else:
