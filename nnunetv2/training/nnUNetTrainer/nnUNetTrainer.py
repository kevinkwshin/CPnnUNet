...
class nnUNetTrainer(object):
    ...
    def _do_i_compile(self):
        # For debugging ProbabilisticUNet, temporarily disable torch.compile
        return False
    ...
    def _build_loss(self):
        ...
        if self.enable_deep_supervision:
            ...
            weights = np.array([1 / (2 ** i) for i in range(len(deep_supervision_scales))]).flatten().astype(np.float32)
    ...
    def get_dataloaders(self):
        ...
        # Reduce the number of processes for data augmentation to avoid resource limits
        # You can adjust this value (e.g., 2, 4) based on your system's capabilities
        num_processes_for_da = 1 # Set to 1 for debugging, increase if stable

        if num_processes_for_da == 0:
            mt_gen_train = SingleThreadedAugmenter(dl_tr, None)
            mt_gen_val = SingleThreadedAugmenter(dl_val, None)
        else:
            mt_gen_train = NonDetMultiThreadedAugmenter(data_loader=dl_tr, transform=None,
                                                        num_processes=num_processes_for_da,
                                                        num_cached=max(6, num_processes_for_da // 2), seeds=None,
                                                        pin_memory=self.device.type == 'cuda', wait_time=0.002)
            mt_gen_val = NonDetMultiThreadedAugmenter(data_loader=dl_val,
                                                      transform=None, num_processes=max(1, num_processes_for_da // 2),
                                                      num_cached=max(3, num_processes_for_da // 4), seeds=None,
                                                      pin_memory=self.device.type == 'cuda',
                                                      wait_time=0.002)
        # # let's get this party started
        _ = next(mt_gen_train)
        _ = next(mt_gen_val)
        return mt_gen_train, mt_gen_val, dl_tr.num_annotators
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        mod.unet.decoder.deep_supervision = enabled
