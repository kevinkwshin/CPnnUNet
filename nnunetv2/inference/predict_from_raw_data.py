...
from nnunetv2.training.nnUNetTrainer.variants.probabilistic_trainer.ProbabilisticUNetTrainer import ProbabilisticUNetTrainer, ProbabilisticUNet


nnUNet_raw = '/workspace/data/project/nnUNet_global/nnUNet_raw'
nnUNet_preprocessed = '/workspace/data/project/nnUNet_global/nnUNet_preprocessed'
nnUNet_results = '/workspace/data/project/nnUNet_global/nnUNet_results'
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        self.num_annotators = checkpoint['init_args'].get('num_annotators', 4)
        ...
        if self.trainer_name == 'ProbabilisticUNetTrainer':
            base_unet = ProbabilisticUNetTrainer.build_network_architecture(
            ...
            self.network = ProbabilisticUNet(
                base_unet, 
                num_annotators=self.num_annotators, 
                latent_dim=6, 
                num_channels=self.label_manager.num_segmentation_heads, 
                patch_size=self.configuration_manager.patch_size, 
                final_conv_channels=self.configuration_manager.network_arch_init_kwargs['features_per_stage'][-1], 
                device=self.device
            )
        else:
            trainer_class = recursive_find_python_class(join(nnunetv2.__path__[0], "training"), self.trainer_name, 'nnunetv2.training')
            if trainer_class is None:
                raise RuntimeError(f'Unable to locate trainer class {self.trainer_name} in {join(nnunetv2.__path__[0], "training")}. '
        ...
        # [MODIFICATION] strict=False를 추가하여, 모델 구조 변경 후에도 이전 가중치를 유연하게 불러올 수 있도록 합니다.
        # 새로 추가된 레이어(z_to_skip_features)의 가중치는 없으므로 초기값을 사용하게 됩니다.
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for params in self.list_of_parameters:
            ...
            if self.annotator_id is not None and self.num_annotators is not None:
                # Create one-hot encoded annotator_id
                # Assuming batch_size is data.shape[0]
                batch_size = data.shape[0]
                annotator_id_tensor = torch.full((batch_size,), self.annotator_id, dtype=torch.long, device=self.device)
                annotator_id_one_hot = nn.functional.one_hot(annotator_id_tensor, num_classes=self.num_annotators).float()
                
                if prediction is None:
                    prediction = self.predict_sliding_window_return_logits(data, annotator_id=annotator_id_one_hot).to('cpu')
                else:
                    prediction += self.predict_sliding_window_return_logits(data, annotator_id=annotator_id_one_hot).to('cpu')
            else:
                if prediction is None:
    ...
    @torch.inference_mode()
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor, annotator_id: Optional[torch.Tensor] = None) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        
        # Initial prediction
        prediction_output = self.network(x, annotator_id=annotator_id)
        if isinstance(prediction_output, dict):
            prediction = prediction_output.get('seg_output', prediction_output)
        else:
            prediction = prediction_output

        # If deep supervision is enabled, network returns a list. We take the highest resolution output.
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                ...
                mirrored_prediction_output = self.network(mirrored_x, annotator_id=annotator_id)
                
                if isinstance(mirrored_prediction_output, dict):
                    mirrored_prediction = mirrored_prediction_output.get('seg_output', mirrored_prediction_output)
                else:
                    mirrored_prediction = mirrored_prediction_output
                
                # handle list output for mirrored prediction as well
                if isinstance(mirrored_prediction, list):
                    mirrored_prediction = mirrored_prediction[0]
    ...
    def _internal_predict_sliding_window_return_logits(self,
                                                       ...
                                                       annotator_id: Optional[torch.Tensor] = None
                                                       ):
            ...
            with tqdm(desc=None, total=len(slicers), disable=not self.allow_tqdm) as pbar:
                while True:
                    ...
                    if item == 'end':
                        ...
                        break
                    workon, sl = item
                    prediction = self._internal_maybe_mirror_and_predict(workon, annotator_id=annotator_id).squeeze(0).to(results_device)
    ...
    @torch.inference_mode()
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, annotator_id: Optional[torch.Tensor] = None)             -> Union[np.ndarray, torch.Tensor]:
...
def predict_entry_point_modelfolder():
    ...
    # Add dummy arguments to accept the user's command format without errors
    parser.add_argument('-d', type=str, required=False, help='Dummy argument for device, not used.')
    parser.add_argument('-c', type=str, required=False, help='Dummy argument for configuration, not used.')
    ...
    parser.add_argument('--disable_progress_bar', action='store_true', required=False, default=False,
                        help='Set this flag to disable progress bar. Recommended for HPC environments (non interactive '
                             'jobs)')
    parser.add_argument('-annotator_id', type=int, required=False, default=None,
                        help='Specify the annotator ID for ProbabilisticUNet inference. Only used if the model is a ProbabilisticUNet.')
