...
class DatasetFingerprintExtractor(object):
    def __init__(self, dataset_name_or_id: Union[str, int], num_processes: int = 8, verbose: bool = False):
        ...
        # The following line is problematic for multi-annotator cases, so we build self.dataset manually
        ...
        if 'training' in self.dataset_json:
            for tr in self.dataset_json['training']:
                # From './imagesTr/case0000_0000.nii.gz' -> 'case0000'
                case_identifier = os.path.basename(tr['image']).split('.')[0][:-len('_0000')]
                
                # Create full paths for images
                image_paths = [join(self.input_folder, i) for i in tr['image']] if isinstance(tr['image'], list) else [join(self.input_folder, tr['image'])]

                # Create full path for the representative label
                label_info = tr['label']
                if isinstance(label_info, list):
                    # For fingerprinting, we only need one label. Let's use the first one.
                    label_path = join(self.input_folder, label_info[0])
                else:
                    label_path = join(self.input_folder, label_info)

                self.dataset[case_identifier] = {
                    'images': image_paths,
                    'label': label_path
                }
        else:
            # fallback for old nnunetv1 dataset format that does not have the 'training' key
            self.dataset = get_filenames_of_train_images_and_targets(self.input_folder, self.dataset_json)
        self.dataset = {}
        for tr in self.dataset_json['training']:
            # From './imagesTr/case0000_0000.nii.gz' -> 'case0000'
            case_identifier = os.path.basename(tr['image']).split('.')[0][:-len('_0000')]
            image_path = join(self.input_folder, tr['image'])
        ...
        # Manually construct the dataset dictionary from dataset.json to support multi-annotator labels
        self.dataset = {}
        for item in self.dataset_json['training']:
            image_path = join(self.input_folder, item['image'])
            label_info = item['label']
            
            # Use the first label as representative for multi-annotator cases
            if isinstance(label_info, list):
                label_path = join(self.input_folder, label_info[0])
            else:
                label_path = join(self.input_folder, label_info)

            case_id = os.path.basename(item['image']).split('.')[0][:-5] # e.g. case0000_0000 -> case0000
            self.dataset[case_id] = {'images': [image_path], 'label': label_path}
        ...
        if not isfile(properties_file) or overwrite_existing:
            ...
            with multiprocessing.get_context("spawn").Pool(self.num_processes) as p:
                for case in self.dataset_json['training']:
                    image_files = [join(self.input_folder, f) for f in case['image']] if isinstance(case['image'], list) else [join(self.input_folder, case['image'])]
                    label_file = case['label']
                    if isinstance(label_file, list):
                        label_file = label_file[0] # Use first annotator as representative
                    label_file = join(self.input_folder, label_file)

                    r.append(p.starmap_async(DatasetFingerprintExtractor.analyze_case,
                                             ((image_files, label_file, reader_writer_class,
