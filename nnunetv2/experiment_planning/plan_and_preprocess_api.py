...
def plan_experiments(experiment_planner_or_dataset_ids: Union[List[int], ExperimentPlanner]):
    """
    Can be a list of dataset ids or a configured ExperimentPlanner
    """
    if isinstance(experiment_planner_or_dataset_ids, ExperimentPlanner):
        experiment_planner_or_dataset_ids.plan_experiment()
        return

    for d in experiment_planner_or_dataset_ids:
        # we do not set the GPU memory target here. If the user wants to do that they can do that by calling
        # ExperimentPlanner directly
        planner = ExperimentPlanner(d, gpu_memory_target_in_gb=8)
