...
def verify_dataset_integrity(folder: str, num_processes: int = 8) -> None:
    ...
    if 'dataset' in dataset_json.keys():
        ...
        # this is specific for our multi-annotator case and will probably not work in all other cases
        all_label_files = subfiles(join(folder, 'labelsTr'), suffix=file_ending, join=False)
        # we need to check if all cases in dataset have at least one label file
        labels_present = []
        for case_id in dataset.keys():
            # this is slow but whatever, this is not a bottleneck
            found_label_for_case = any([i.startswith(case_id) for i in all_label_files])
            labels_present.append(found_label_for_case)
