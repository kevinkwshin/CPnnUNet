...
class DefaultPreprocessor(object):
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        # Manually build the list of cases to preprocess to support multi-annotator labels
        cases_to_process = []
        if 'training' in dataset_json:
            for tr in dataset_json['training']:
                image_files = [join(nnUNet_raw, dataset_name, i) for i in tr['image']] if isinstance(tr['image'], list) else [join(nnUNet_raw, dataset_name, tr['image'])]
                
                label_info = tr['label']
                if isinstance(label_info, list):
                    # Create a separate preprocessing job for each annotator
                    for label_file in label_info:
                        # From './labelsTr/case0000_annotator1.nii.gz' -> 'case0000_annotator1'
                        unique_identifier = os.path.basename(label_file).split('.')[0]
                        output_filename_truncated = join(output_directory, unique_identifier)
                        cases_to_process.append((output_filename_truncated, image_files, join(nnUNet_raw, dataset_name, label_file)))
                else:
                    unique_identifier = os.path.basename(tr['image']).split('.')[0][:-len('_0000')]
                    output_filename_truncated = join(output_directory, unique_identifier)
                    cases_to_process.append((output_filename_truncated, image_files, join(nnUNet_raw, dataset_name, label_info)))
        else:
            # Fallback for old format
            dataset = get_filenames_of_train_images_and_targets(join(nnUNet_raw, dataset_name), dataset_json)
            for k in dataset.keys():
                output_filename_truncated = join(output_directory, k)
                cases_to_process.append((output_filename_truncated, dataset[k]['images'], dataset[k]['label']))

        # multiprocessing magic.
        r = []
        with multiprocessing.get_context("spawn").Pool(num_processes) as p:
            remaining = list(range(len(cases_to_process)))
            # p is pretty nifti. If we kill workers they just respawn but don't do any work.
            # So we need to store the original pool of workers.
            workers = [j for j in p._pool]
            for i, (output_fname, image_f, label_f) in enumerate(cases_to_process):
                r.append(p.starmap_async(self.run_case_save,
                                         ((output_fname, image_f, label_f,
                                           plans_manager, configuration_manager,
                                           dataset_json),)))

            with tqdm(desc=None, total=len(cases_to_process), disable=self.verbose) as pbar:
