...
class nnUNetDataLoader(DataLoader):
    def __init__(self,
        ...
        preprocessed_dataset_folder = os.path.dirname(self._data.source_folder)
        dataset_json_path = join(preprocessed_dataset_folder, 'dataset.json')
        self.dataset_json = load_json(dataset_json_path)

        self.case_id_to_labels = {os.path.basename(d['image']).split('.')[0][:-len('_0000')]: d['label']
                                  for d in self.dataset_json['training']}

        all_annotators = set()
        for labels in self.case_id_to_labels.values():
            if isinstance(labels, list):
                for label_file in labels:
                    annotator_id = os.path.basename(label_file).split('.')[0].split('_')[-1]
                    all_annotators.add(annotator_id)
        
        if all_annotators:
            self.annotator_mapping = {name: i for i, name in enumerate(sorted(list(all_annotators)))}
            self.num_annotators = len(all_annotators)
            if self.verbose:
                print(f"Found {self.num_annotators} annotators: {self.annotator_mapping}")
        else:
            self.annotator_mapping = None
    ...
    def generate_train_batch(self):
        ...
        # for multi-annotator support
        annotator_ids_all = np.zeros((self.batch_size, self.num_annotators), dtype=np.float32) if self.num_annotators > 0 else None
        ...
        for j, i in enumerate(selected_keys):
            ...
            if self.num_annotators > 0:
                labels_for_case = self.case_id_to_labels.get(i)
                if isinstance(labels_for_case, list):
                    # Randomly select one annotator's label for this sample
                    selected_label_file = random.choice(labels_for_case)
                    target_label_file = os.path.basename(selected_label_file)
                    
                    # Create one-hot encoded annotator ID
                    annotator_id_str = target_label_file.split('.')[0].split('_')[-1]
                    annotator_int_id = self.annotator_mapping[annotator_id_str]
                    annotator_ids_all[j, annotator_int_id] = 1
                # if it's not a list, it's a single label, proceed as normal

            data, seg, seg_prev, properties = self._data.load_case(i, target_label_file)
        ...
        if self.transforms is not None:
            ...
            if annotator_ids_all is not None:
                batch['annotator_id'] = torch.from_numpy(annotator_ids_all).float()
            return batch

        batch = {'data': data_all, 'target': seg_all, 'keys': selected_keys}
        if annotator_ids_all is not None:
            batch['annotator_id'] = annotator_ids_all
        return batch
